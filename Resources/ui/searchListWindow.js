//Book Table View. Top Rentals.	
	...
	var local= new function(){
  	...
  	//Ti.App.search_allbooks = [];
function searchListWindow(searchText) {	
	...
	function loadData() {
		tableData=[];				
		for (var i = 0, l = Ti.App.search_books.length; i < l; i++) {
   	        var book = Ti.App.search_books[i];
			var row ;
			if(i%2==0){
			...
			{
					displayDetails(Ti.App.search_books , e.row.id);
			})
	...
	actInd.show();
	network.searchList(searchText,function(books1) {
		for (var i = 0, l = books1.length; i<l; i++) {
			Ti.App.search_books.push(books1[i]);
			//Ti.App.search_allbooks.push(books1[i]);
		} 	 
		actInd.hide();
					loadData();
					...
					height=Ti.App.maxResults*101;
					table.setHeight(height);
					
		//Ti.App.fireEvent("loaded2", {message:"branch"});	
	});
...
function displayDetails(search_books,no1)
	{           
		       // alert(no1);
		       //alert(search_books.length)
				var no = no1;
				var top=0;
				if(Ti.App.search_books[no].description.length<1)
				{
					Ti.App.search_books[no].description="No Description available for this book"
		 		}
				...
				{
					//url:Ti.App.search_allbooks[no].imageURL,
					defaultImage:'../images/noimage.jpg',
					left  : 5,
					top   :10,
					width :60,
			  ...
			  nextbutton.addEventListener('click',function()
				{			
					...
					{
						if(parseInt(no) == Ti.App.maxResults-1)
						{	image.setUrl(Ti.App.search_books[no].imageURL);
							bookTitle.setText(Ti.App.search_books[no].title);
							bookAuthor.setText(  "Authour  : "+Ti.App.search_books[no].author);
							bookCategory.setText("Category : "+Ti.App.search_books[no].category.toString());							
						}
						else
						{
							image.setUrl(Ti.App.search_books[no].imageURL);
			...
			prevButton.addEventListener('click',function()
			   ...
			   backButton.addEventListener('click',function()
				...
				var bookTitle = Ti.UI.createLabel({
					text:Ti.App.search_books[no].title,
					top:1,
					left:10,
					color:"#000000",
					height:'auto',
			   ...
			   var bookAuthor = Ti.UI.createLabel({
					text:'Author : '+Ti.App.search_books[no].author,
					top:1,
					left:10,
					color:"#000000",
					height:'auto',
			   ...
			   var bookCategory= Ti.UI.createLabel({
					text:'Category : ' + Ti.App.search_books[no].category.toString(),
					top:1,
					left:10,
					color:"#000000",
					height:'auto',
			   ...
			   var bookDesc=Ti.UI.createLabel(				
					{
						text:Ti.App.search_books[no].description,
						//text : 'scrollView.add(bookDescTitle)crollView.add(bookDesc)bookInnerView.add(image);				bookInnerView.add(bookDetailsView);				bookInnerView.add(buttonView);				bookInnerView.add(scrollView);				bookView.add(bookInnerView);				displayWindow.add(bookView);',												    
					...
					{		
						...
						actInd.show();			
						network.rentBooks(Ti.App.search_books[no].id,function(data) {
							 actInd.hide();
							 alert(data);	
							 displayWindow.close();						 							 
						 });			
...
module.exports = searchListWindow;
